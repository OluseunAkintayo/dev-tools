...
const XmlToJson = () => {
  ...
  const convertXmlToJson = (e: React.FormEvent) => {
    e.preventDefault();
    const parser = new DOMParser();
    const xmlDoc = parser.parseFromString(xmlData, "text/xml");

    function convertNode(node: unknown) {
          ...
          if (child.nodeType === Node.ELEMENT_NODE) {
            ...
            if (obj[nodeName]) {
              if (!Array.isArray(obj[nodeName])) {
                (obj as any)[nodeName] = [(obj as any)[nodeName]];
              }
              (obj as any)[nodeName].push(childObj);
            } else {
  ...
  const upload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      console.log((fileContent as string).split("\n"));
    };
    ...
    <LandingPageLayout>
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="sm:max-w-[600px] mx-auto">
          <form onSubmit={convertXmlToJson} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button disabled={loading || !xmlFile} type="submit">
